...
function getmstz(){
var tz=document.ms.owata.options[document.ms.owata.selectedIndex].text;
	var tzm =tz.match(/-?\d+/g);
	var t_offset=tzm[0];
	var t_offset_min=tzm[1];
	var jtc=t_offset*1 + t_offset_min/60;
	return jtc;
}
...
var kk=moment(a).clone();
kk=moment.utc(kk).add("Hours",getmstz());
s+="<br>M$TIME;" + kk.format("MM月DD日HH時mm分[GMT]")+Math.trunc(getmstz())+":"+Math.abs(((getmstz()*100)%100)*0.6); 
}
if(document.getElementById("usemoment").checked){
var kt=moment(a).clone();
var tzst=tzJAP[document.gm.gm.selectedIndex][0];
kt=moment(kt).tz(tzst);
s+="<br>MOMENT;" + kt.format("MM月DD日HH時mm分Z,z"); 
}
