...
var xmin = chart.series[0].xAxis.min;
var xmax = chart.series[0].xAxis.max;
            // Make a sortable array
            for (x in rows) {
                if (Object.hasOwnProperty.call(rows, x)) {
if (rows[x].x >= xmin && rows[x].x <= xmax) {
                    rowArr.push(rows[x]);
                    }
                }
            }
        ...
        Highcharts.Chart.prototype.getCSV = function (useLocalDecimalPoint) {
            ...
            rows.forEach(function (row, i) {
                ...
                while (j--) {
                    ...
                    if (val.indexOf("2014-01-")>=0) {
                       if(RM){
		               
                       val = moment(-moment(val)+timediff+ibe_owari);
...
<option>0～ シリアル値VB6,M$エクセル、googleスプレッドシート</option>5
<option>0～ エポック秒</option>6
<option>--イベント時間で出力(現イベ基準)--</option>
<option selected="selected">イベント開始りある時間(現地) realtime</option>8
<option>イベント開始りある時間 toString()英語表記？</option>9
<option>イベント開始りある時間 エクセルシリアル値</option>10
<option>イベント開始りある時間ISO8601 realtime</option>11
<option>イベント開始りある時間ISO8601 unixtime</option>12
<option>イベント開始りある時間 エポック秒</option>13
</select>
*/

                       if(seltxt==5){
                       val +=timezone;
                       val =val.valueOf()/3600/1000/24;
                       }
                       else if(seltxt==6){
                       val +=timezone;
                       ...
                       val = val -moment("1899/12/30 00:00:00");
                       val =val.valueOf()/3600/1000/24;
                       }
                        else if(seltxt==11){
                       val =val.format();
                       ...
                       val = val.format(defaultdate) ;
                       }
                       val='"' +val+'"' ;
                       if(seltxt==1){
                       val =val.replace(/^"2001/gm,"0001");
                       }
                       
                       }
                       else if(DMS){
                       val = moment(moment(val)-moment("2014-01-01T00:00:00Z")+timezone).utc();
                       val ='"' + (parseInt(val.format("D"))-1) +" "+ val.format("HH:mm") +'"';
                       }
                       else{
                       val = '"' + (moment(val)-moment("2014-01-01T00:00:00Z")+timezone)/3600000 + '"';}
                    }
                    else{
                        val = '"' + val + '"';
                    }
        ...
        Highcharts.Chart.prototype.getTable = function (useLocalDecimalPoint) {
            var html = '<table id="highcharts-data-table" class="border">', options = this.options, decimalPoint = useLocalDecimalPoint ? (1.1).toLocaleString()[1] : '.', useMultiLevelHeaders = pick(options.exporting.useMultiLevelHeaders, true), rows = this.getDataRows(useMultiLevelHeaders), rowLength = 0, topHeaders = useMultiLevelHeaders ? rows.shift() : null, subHeaders = rows.shift(), 
...
*/
            ...
            var ttxt=options.title.text.replace(/<.*?>/gm,"");
                html += '<caption class="highcharts-table-caption">' + pick(options.exporting.tableCaption, (ttxt ?
                    htmlencode(ttxt) :
                    'Chart')) + '</caption>';
            }
            // Find longest row
